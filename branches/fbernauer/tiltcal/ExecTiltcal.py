...
parser.add_option('--tilt', type=float, dest='tilt',
                    default=24.6,
                    help='absolute tilt in m/s/s')
parser.add_option('--gap', type=float, dest='gap',
                    default=0.0,
                    help='safety distance from pulse flanks in seconds')
...
raw, deconv_vel, acc, marks = tiltcal(data, samprate, fcs, hs, gap, dgain, step, bfc=0.0, bm=0)
...
ax2 = plt.subplot(312)
ax2.plot(deconv_vel, label='real ground movement')
for i in range(len(marks)):
...
ax3 = plt.subplot(313)
ax3.plot(acc, label='real acceleration')
for i in range(len(marks)):
