...
typedef struct ContinuousSegment_s {
  ...
  int samplecnt;                          // Total sample count
...
typedef struct LinkedIDList_s {
  ...
  struct LinkedIDList_s *last;              // Pointer to the last id
}
LinkedIDList;
...
LinkedIDList * readMSEEDBuffer (char *mseed, int buflen, Selections *selections,
                     flag unpack_data, int reclen, flag verbose, long (*allocData) (int, char))
{
  int retcode = 0;
  int retval = 0;
  flag swapflag = -1;
  ...
  int began_record_list = 0;


  LinkedIDList * idListHead;
  LinkedIDList * idListCurrent;
  ContinuousSegment * segmentHead;
  ContinuousSegment * segmentCurrent;
  hptime_t lastgap;
  hptime_t hptimetol;
  hptime_t nhptimetol;
  long data_offset;
  LinkedRecordList *recordHead;
  LinkedRecordList *previousItem;
  LinkedRecordList *recordCurrent;
  int datasize;
  ...
  while (offset < buflen)
  {
    ...
    if ( began_record_list == 0 )
      {
      recordHead = recordCurrent;
      previousItem = recordCurrent;
      recordCurrent->previous = NULL;
      began_record_list = 1;
      }
    else
      {
      previousItem->next = recordCurrent;
      recordCurrent->previous = previousItem;
      previousItem = recordCurrent;
      }
  ...
  idListHead->last = idListHead;

  // Set the attributes of the ID container and the first segment.
  strcpy(idListCurrent->network, recordHead->record->network);
  strcpy(idListCurrent->station, recordHead->record->station);
  strcpy(idListCurrent->location, recordHead->record->location);
  strcpy(idListCurrent->channel, recordHead->record->channel);
  idListCurrent->dataquality = recordHead->record->dataquality;

  segmentHead = segmentCurrent = seg_init();

  idListCurrent->firstSegment = segmentCurrent;
  idListCurrent->lastSegment = segmentCurrent;

  segmentCurrent->starttime = recordHead->record->starttime;
  segmentCurrent->endtime = msr_endtime(recordHead->record);
  segmentCurrent->samprate = recordHead->record->samprate;
  segmentCurrent->sampletype = recordHead->record->sampletype;
  segmentCurrent->samplecnt = recordHead->record->samplecnt;
  // Calculate high-precision sample period
  segmentCurrent->hpdelta = (hptime_t) (( recordHead->record->samprate ) ?
                       (HPTMODULUS / recordHead->record->samprate) : 0.0);

  segmentCurrent->firstRecord = segmentCurrent->lastRecord = recordHead;

  // The loop will start with the second record.
  recordCurrent = recordHead->next;
  ...
  while (recordCurrent != NULL)
  {
    ...
    if (idListCurrent == NULL)
    {
      ...
      idListCurrent->previous = idListHead->last;
      idListHead->last->next = idListHead->last->last = idListHead->last = idListCurrent;
    ...
    if ( segmentCurrent->sampletype == recordCurrent->record->sampletype &&
    ...
    else
    {
      ...
      idListCurrent->lastSegment = idListCurrent->lastSegment->next = segmentCurrent;
