...
#MS modified for bulk option     
    def getInventory(self, network='*', station='*', location='*', channel='*',\
        ...
        :param bulk: Returns string for bulk select
            Default is None
        :return: Dictionary of inventory information.(or string if bulk is True)
...
#MS
        if bulk:
            data=self.get_inv_for_bulk(rtype, rdata, starttime, endtime)
        else:
#MS end
            # fetch plain XML document
            if network == '*':
...
#MS added
    def get_inv_for_bulk(self, rtype, rdata, starttime, endtime):
        """
        returns list of inventory as string
        """
        rdata[0]=(rdata[0]-1).formatArcLink()
        rdata[1]=(rdata[1]-1).formatArcLink()
        self._hello()
        self._writeln(rtype)
        out=' '.join(rdata)
        self._writeln(out)
        ...
        bulk_inv=str()
        for network in xml_doc.xpath('ns:network', namespaces={'ns': xml_ns}):
            for station in network.xpath('ns0:station', namespaces={'ns0': xml_ns}):
                for stream in station.xpath('ns:' + stream_ns, namespaces={'ns': xml_ns}):
                    for comp in stream.xpath('ns:' + component_ns, namespaces={'ns': xml_ns}):
                        sta = str()
                        sta += starttime.formatArcLink()
                        sta += ' '+endtime.formatArcLink()
                        sta += ' '+network.get('code')
                        sta += ' '+station.get('code')
                        sta += ' '+comp.get('code')
                        if stream.get('code') in '':
                            sta += ' . '
                        else:
                            sta += ' '+stream.get('code')
                        sta += ' \n'
    ...
    def bulk_stations(self, bulk_inv):
        """
        gets station information including PAZ and gain for bulk_inventory list
        return dict with station information
        """
        instruments=True
        # request type
        rtype = 'REQUEST INVENTORY '
        rtype += 'instruments=true '
        # request data
        max_requestlines=900 #arclink seems to have problems with requests above ~ 500. exact number should be found
        i=0
        inv=[]
        #split request into chunks
        while i*max_requestlines<len(bulk_inv.split('\n')):
            inv.append('\n'.join(bulk_inv.split('\n')[i*max_requestlines:(i+1)*max_requestlines]))
            i+=1
        stations={}
        for chunk in inv:
            rdata = chunk
            ...
            for network in xml_doc.xpath('ns:network', namespaces={'ns': xml_ns}):
                for station in network.xpath('ns0:station', namespaces={'ns0': xml_ns}):
                    ...
                    for key in ['code', 'description', 'affiliation', 'country',
                                'place', 'restricted', 'archive_net']:
                        stat[key] = station.get(key, '')
                    # floats
                    for key in ['elevation', 'longitude', 'depth', 'latitude']:
                        try:
                            stat[key] = float(station.get(key))
                        except:
                            stat[key] = None
                    for stream in station.xpath('ns:' + stream_ns, namespaces={'ns': xml_ns}):
                        for comp in stream.xpath('ns:' + component_ns, namespaces={'ns': xml_ns}):
                            ...
                            if len(paz_id) == 0:
                                try:
                                    paz_id = xml_doc.xpath('ns:' + seismometer_ns + \
                                                   '[@' + name_ns + '="' + \
                                                   stream.get(seismometer_ns, None) + '"]/@response',
                                                   namespaces={'ns': xml_ns})
                                except:
                                    #print 'no PAZ for ', seismometer_id #################################no paz or could i just not find it? seismometer_id =/= sensor name
                                    temp['paz']='no paz'
                                    pass
                            else:
                                #print seismometer_id
                                try:
                                    paz_id = paz_id[0]
                                except:
                                    continue
                            ...
                            if stream.get('code') in '':
                                ...
                                sta += ' '+stream.get('code')
                            stations[sta]=dict(temp.items()+stat.items())
        return stations
    
    def bulkdatarouting(self, bulk_inv):
        """
        :type bulk_inv: string 
        :param:list of stations, channels networks, as defined in get_bulk_inv()
        
        return: inv_by_route: a dictionary of hosts with accociated bulk_inventories
        """
        # request type
        rtype = 'REQUEST ROUTING '
        # request data
        ...
        hosts=set([(result.get(key)[0]['host'], result.get(key)[0]['port']) for key in result.keys()])
        hosts=list(hosts)
        #for each unique host (hostname and port), list all associated networks in routing dictionary
        routing={}
        for host in hosts:
            routing[host]=[network.split('...')[0] for network in result if host[0] in result.get(network)[0]['host'] and host[1]==result.get(network)[0]['port']]
        #return dictionary of bulk_inventories associated to unique hosts
        inv_by_route={}
        for route in routing:
            inv_by_route[route]=list()
            for network in routing.get(route):
                #add all stations form bulk_inv that contain network code
                stations_in_network=[line for line in bulk_inv.split('\n') if len(line)>2 and network in line.split(' ')[2]]
                inv_by_route[route].extend(stations_in_network)
        return inv_by_route
 
    def bulkdataselect_on_host(self, bulk_inv, filename, format_='MSEED', compressed=False, unpack=True, route=('host', 'port')):
        """
        """
        # request type
        request_type = 'REQUEST WAVEFORM format=%s compression=%s"' % (format_,compressed)
        if route is not ('host', 'port'):
            try:
                self._client.host = host[0]
                self._client.port = host[1]
                self._reconnect()
            except:
                pass
        self._hello()
        ...
        while True:
            ...
            time.sleep(1)
        # check for errors
        blub=deepcopy(xml_doc)
        not_in=[available for available in blub.split('\n') if not 'status="OK"' in available]
        xml_doc='\n'.join(xml_doc.split('\n')[:3])+'\n'+'\n'.join([available for available in xml_doc.split('\n') if 'status="OK"' in available])+'\n'+'\n'.join(xml_doc.split('\n')[-4:]) #VERY DIRTY!!
        for err_code in ['DENIED', 'CANCELLED', 'CANCEL', 'ERROR', 'RETRY',
                         'WARN', 'UNSET']:
            err_str = 'status="%s"' % (err_code)
            if err_str in xml_doc:
    ...
    def bulkdataselect(self, bulk_inv, filename='tmp', format_='MSEED', compressed=False, unpack=True, host='initial host'):
        """
        """
        max_requestlines=980 #arclink seems to have problems with requests above ~ 900. exact number should be found
        i=0
        #inv=[]
        #split request into chunks
        ...
        while i*max_requestlines<len(bulk_inv.split('\n')):
            ...
            #inv.append(chunk)
            i+=1
            inv_by_route=self.bulkdatarouting(chunk)
            #import ipdb; ipdb.set_trace()
            for host in inv_by_route:
                inv='\n'.join(inv_by_route[host])
                #import ipdb; ipdb.set_trace()
                try:
                    self.bulkdataselect_on_host(inv, filename='tmp.'+format_, route=host, format_=format_)
                    st+=read('tmp.'+format_)
                except:
                    msg = "Could not download waveform data from host %s."
                    warnings.warn(msg % (host[0]))
                    pass
        #st.merge()
        st.write(filename+format_, format_)
        return st
...
if __name__ == '__main__':
    pass
#    c=Client()
#    inv=c.getInventory(channel='BHZ', starttime=UTCDateTime(2001, 4,6),endtime=UTCDateTime(2001, 4,6)+3000, min_latitude=30, max_latitude=90, min_longitude=-30, max_longitude=40, bulk=True)+\
#    c.getInventory(channel='BHN', starttime=UTCDateTime(2001, 4,6),endtime=UTCDateTime(2001, 4,6)+3000, min_latitude=30, max_latitude=90, min_longitude=-30, max_longitude=40, bulk=True)+\
#    c.getInventory(channel='BHE', starttime=UTCDateTime(2001, 4,6),endtime=UTCDateTime(2001, 4,6)+3000, min_latitude=30, max_latitude=90, min_longitude=-30, max_longitude=40, bulk=True)
#    stations=c.bulk_stations(inv)
#   st2=c.bulkdataselect_on_host(inv, "firsthost.mseed") ### WARNING, can be too many requests. check if len(inv.split('\n'))<900 !!!
#    st=c.bulkdataselect(bulk_inv=inv, filename='OUTPUT/CENTRAL_ITALY_C200904060132A/seis.mseed', format_='MSEED')
#MS End
